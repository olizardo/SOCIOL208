...
We can use a variant of the [spectral clustering approach](comm-spectral.qmd) to find communities in two-mode networks [@wu_etal22]. This approach combines [Correspondence Analysis (CA)](tm-ca.qmd) and k-means clustering on multiple dimensions. 
...
Of course, we can't use the `mod.check` function [we used before](comm-spectral.qmd) because that relies on the standard method for checking the modularity in one-mode networks and doesn't take into account the structural zeros in the bipartite graph. 
...
Here we create the `U` matrix from the first two dimensions of the eigendecomposition of the bipartite modularity matrix. The results suggest that the three community partition is optimal, although the two-community one also does well. The corresponding splits are shown in and . 
