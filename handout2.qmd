...
The interesting cases come from $1 > \delta < \infty$ and $0 > \delta < 1$. In the first case, nodes at shorter distances are weighted more (like in the standard harmonic centrality measure) as \delta becomes bigger and bigger then the measure approximates degree. For values below one, as $\delta$ approaches zero then being connected to other nodes counts equally for your centrality regardless of how far away they are. 

Let us go back to the original under-forty women lawyers network to see how this works:
...
The first version of the harmonic centrality, with a positive value of $\delta$ above zero, will compute centrality scores emphasizing *direct* (one-step) connections, thus coming closer to **degree**. The second version with a value of $\delta$ close to zero, will emphasizing *indirect* connections weighing longer paths almost as much as shorter paths Finally, the last version, with $\delta < 0$, will weigh *longer* paths more than short ones, serving as a measure of **eccentricity** (farness from others) not closeness. 
