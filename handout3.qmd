...
So the idea is to combine the HITS reflective centrality idea with PageRank's degree-weighting scheme. We can do this by using a variation of the status game we used for HITS, developed by @lempel_moran01.^[They called their approach SALSA an acronym for the also impossible to remember title of "Stochastic Approach for Link Structure Analysis."] Instead of using the original adjacency matrix $\mathbf{A}$ and its transpose $\mathbf{A}^T$ we use a couple of degree weighted variations:

$$
\mathbf{P}^h = \mathbf{D}_{(in)}^{-1}\mathbf{A}^T
$$

$$
\mathbf{P}^a = \mathbf{D}_{(out)}^{-1}\mathbf{A}
$$

Where $\mathbf{D}_{(in)}^{-1}$ is a **diagonal matrix** containing the inverse of the indegrees of each node ($1/\sum_ia_{ij}$) along the diagonals and zeros everywhere else (the inverse of the diagonals of $\mathbf{N}$ above) and $\mathbf{D}_{(out)}^{-1}$ is another diagonal matrix containing the inverse of the outdegrees of each node ($1/\sum_ja_{ij}$) along the diagonals and zeros everywhere else (the inverse of the diagonals of $\mathbf{M}$ above).

Our new version of the HITS status game is then played using these matrices:

$$  
   x^h_i = \sum_j p^h_{ij} x^a_j
$$

$$
   x^a_i = \sum_i p^a_{ij} x^h_i
$$
...
```{r}
   status3 <- function(A, e = 0.000001) { 
      ...
      a <- rep(1, n)  #initializing authority scores
      Pa <- A/rowSums(A) #row normalized adjacency matrix
      Ph <- t(A)/colSums(A) #row normalized transpose of adjacency matrix
      diff <- 1
      while (diff > e) {
         o.h <- h
         a <- t(Pa) %*% h #authority scores a function of current hub scores weighted by the outdegree of the hubs pointing to the authority
         a <- a/norm(a, type = "E")
         h <- t(Ph) %*% a #hub scores a function of current authority scores weighted by the indegree of the authorities the hub points to
         h <- h/norm(h, type = "E")
         diff <- abs(sum(abs(o.h) - abs(h)))
         }
      return(list(a = as.vector(a), h = as.vector(h)))
      }
```
...
Neat! Also, like with HITS, it is clear that these scores are the leading eigenvectors of two matrices $\mathbf{M^h}$ and $\mathbf{M^a}$, given by the product of $\mathbf{P^h}$ and $\mathbf{P^a}$:

$$
\mathbf{M}^h = (\mathbf{P^h})^T(\mathbf{P^a})^T
$$

$$
\mathbf{M}^a = (\mathbf{P^a})^T(\mathbf{P^h})^T
$$
...
```{r}
   ...
   M.h <- t(Ph) %*% t(Pa)
   M.a <- t(Pa) %*% t(Ph)
   h <- Re(eigen(M.h)$vectors[,1]) *-1 
   a <- Re(eigen(M.a)$vectors[,1]) *-1
   round(h/max(h), 2)
   round(a/max(a), 2)
```
...
Like before, we can treat the PageRank, Hub, and Authority Scores (regular and SALSA) as "centralities" defined over nodes in the graph. In that case we might be interested in how different nodes stack up according to the different status criteria:

```{r}
#| echo: false
   nodes <- 1:vcount(g)
   pr.cent <- round(status1(t(G))/max(status1(t(G))), 2)
   hu.cent <- round(h/max(h), 2)
   au.cent <- round(a/max(a), 2)
   shu.cent <- round(salsa$h/max(salsa$h), 2)
   sau.cent <- round(salsa$a/max(salsa$a), 2)
   cent.dat <- data.frame(Nodes = nodes, PageRank = pr.cent, 
                          Hub.Score = hu.cent, Auth.Score = au.cent, 
                          SALSA.hub = shu.cent, SALSA.Auth = sau.cent, 
                          Indegree = degree(g, mode = "in"))
   cent.dat <- cent.dat[order(cent.dat$Indegree, decreasing = TRUE), ]
   kbl(cent.dat[1:10, ], 
       format = "html", align = "c", row.names = FALSE,
       caption = "Top Ten Status and Prestige Scores Ordered by Indegree.") %>%    kable_styling(bootstrap_options = c("hover", "condensed", "responsive"))
   
   cent.dat <- data.frame(Nodes = nodes, PageRank = pr.cent, 
                          Hub.Score = hu.cent, Auth.Score = au.cent, 
                          SALSA.hub = shu.cent, SALSA.Auth = sau.cent, 
                          Outdegree = degree(g, mode = "out"))
   cent.dat <- cent.dat[order(cent.dat$Outdegree, decreasing = TRUE), ]
   kbl(cent.dat[1:10, ], 
       format = "html", align = "c", row.names = FALSE,
       caption = "Top Ten Status and Prestige Scores Ordered by Outdegree") %>%    kable_styling(bootstrap_options = c("hover", "condensed", "responsive"))
```
