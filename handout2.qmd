...
Note that the reachability relation is asymmetric: So node 4 and reach node 1 (there is a directed tie from 4 to 1), but node 1 cannot reach 4. This is precisely what we want. 
...
This tells us that the betweenness of an edge *e* is the ratio of the number of times that edge appears in the middle of a shortest path connecting every pair of nodes in the graph *i* and *j* divided by the total number of shortest paths linking every pair of nodes. Like before, this is a probability: Namely, that if you send something from *i* to *j* it has to go through edge *e*. 
...
The "collapse" value in the "mode" argument tells `igraph` to take every edge and transform it into an undirected one, collapsing mutual edges into one ignoring direction and removing the arrows from the one-sided ones. 
...
High betweenness edges are likely to function as **bridges** being the only point of connection between most nodes in the social structure. That means that an easy way to disconnect a graph is to remove the bridges [@girvan_newman02].
