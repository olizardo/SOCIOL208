...
So to get the centered network adjacency matrix, we first need to compute the corresponding **column means** of the matrix. Note that this will be equivalent to each individual's **indegree** (the sum of the columns) divided by the total number of nodes in the network ($k^{in}_i/n$):
...
Note that to subtract the column mean vector from each column of the adjacency matrix we first transpose it, do the subtraction, and "untranspose" the resulting matrix back. 

Note that indeed, the column means of the centered adjacency matrix are zero:
...
And the resulting normalized scores for each node in the Krackhardt managers advice network are:
...
Note that we set the argument `scale.unit` to `FALSE` so that the PCA is conducted on the original adjacency matrix and not a scaled version of it!
...
Let's check that out in our case. First, we load the package `FactoMineR` which can be used to compute the correspondence analysis of any matrix in `R` using the function `CA` from the `FactoMineR` package. 

The correspondence analysis scores---on the first dimension---fo the Krackhardt advice network can be obtained like this:
