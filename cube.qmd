...
We name the function `cube`. Lines 2-5 it's just a mini-function called `kill.ends` within the main function that takes a vector as input and returns the same vector minus the first and last element as output. This will come in handy when we want to extract the **inner nodes** (brokers) of the path information returned by the `igraph` function `all_shortest_paths` while dropping the sender and receiver nodes. 
...
Lines 9-19 populate the cube with the required information using an (inefficient as noted earlier) triple `for` loop. As noted some, useful `igraph` functions come into play here:

- In line 12 the `if` conditional inside the triple loop uses the `neighbors` function in `igraph` and checks that node $r$ is not a neighbor of $s$ (if they are directly connected then node $b$ cannot be a broker). 

- After we check that $s$ and $r$ are not neighbors, we use the `all_shortest_paths` function in `igraph` to get all the shortest paths between $s$ and $r$ in line 13. 

- Line 14 uses the `kill.ends` function to drop the source and receiver nodes from the vector of node ids returned by the `all_shortest_paths` function.

- Line 15 uses some `sapply` magic and the base `R` function `%in%` to check how many times each broker node $b$ shows up in that list of shortest paths as an inner node between $s$ and $r$; we put that number in the $ijk^{th}$ cell of the array, and loop through all triplets until we are done. 

Finally, lines 21-23 loop through the cube again, this time to take the ratio of the number of shortest paths involving each broker node to the *total* number of shortest paths between each pair (the pairwise betweenness), which we computed in line 20 using our trusty `nsp` function. So all we have to do is divided each eight by eight matrix in the cube by the $S$ matrix. 
...
Of course, closeness is also in the cube because of the mathematical relationship we saw earlier between the sum of entries between senders and receives across brokers in the cube and the geodesic distance. 

So if we wanted to get the geodesic distances between every pair of nodes in the graph, all we need to do is loop through each pair of nodes and compute the relevant sum across brokers:
...
Which is the same as the geodesic distance matrix:
