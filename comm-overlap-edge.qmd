...
- In line 5, we extract the end nodes incident to each edge in the graph using the `igraph` function `ends` (which returns an $E \times 2$ matrix the nodes at end of the edges in one column and the nodes at the other end of the edges in the other column). 
...
The leaves of the dendrogram (bottom-most objects) represent each link in the graph ($E =$ `r ecount(g)` in this case), and the clusters are "link communities" [@ahn_etal10].
...
#| fig-width: 12
#| echo: false
    W_raw <- split(B/rowSums(B), seq(nrow(B)))
    W <- lapply(W_raw, function(x) { x[x != 0] })
    ...
    legend("bottomright", 
       legend = c(paste("Commnunity", 1:ncol(B), sep = "_")), 
       fill = pal, 
