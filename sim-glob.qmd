...
Where $e$ refers to the [matrix exponential](https://en.wikipedia.org/wiki/Matrix_exponential) operation. 
...
Note that we set the alpha parameter to $\alpha = 1.5$. When we choose a value of $\alpha$ closer to zero ($\alpha \rightarrow 0$), longer paths get discounted a lot; this means that the weight of the measure is borne by the degree of each node (direct links). As $\alpha$ increases, we give more weight to indirect paths [@benzi_klymko14].
