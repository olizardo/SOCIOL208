...
@brohl_lehnertz22 propose to generalize the concept of degree centrality to the edge level. The basic idea is to consider an edge to be central if (1) its incident nodes also have high degree centrality, and (2) if the difference between the degrees of the two nodes is relatively small (e.g., the centrality of the edge is not driven by only one of the two nodes). This leads to the measure:

$$
C^{DEG}(e_{jk}) = \frac{C^{DEG}(v_j) + C^{DEG}(v_k) - 2}{|C^{DEG}(v_j) - C^{DEG}(v_k)| + 1}
$$ {#eq-edeg}

Where the numerator of @eq-edeg is just the sum of the degrees of nodes $i$ and $j$ minus 2 and the denominator is the absolute value of the differences of the degrees plus 1. 
