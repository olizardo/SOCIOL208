...
Which is obtained by "vectorizing" the matrices and computing the Pearson correlation between the vector corresponding to friendship ties and the vector corresponding to advice ties. Here, we see that there is a positive correlation between the two ($r = 0.26$) suggesting people tend to seek advice from friends (or befriend their advisors/advisees, as correlations don't imply directionality).
...
For instance the original adjacency matrix of the Krackhardt Managers friendship nomination network is:
...
The new permuted matrix looks like:
...
So let's say we wanted to predict advice ties from friendship ties while adjusting for the present of formal reporting ties. We could just a regression like this:
...
```{r}
   run.reg <- function(g, xvars, dat) {
      A <- as.matrix(as_adjacency_matrix(g))
      dat <- data.frame(y.perm = as.vector(A), dat)
      reg.res <- lm(reformulate(xvars, "y.perm"), data = dat)
      return(as.vector(reg.res$coefficients))
   }
```

And now we run the regression on our 1000 strong ensemble of permuted friendship network graphs:

```{r}
   reg.list <- lapply(G3, run.reg, xvars = c("adv", "rep"), dat = ht.dat)
...
```{r}
   ...
   reg.tab <- cbind(reg.res$coefficients, p.vals)
   rownames(reg.tab) <- c("Intercept", "Friend Tie", "Formal Tie")
   colnames(reg.tab) <- c("Estimate", "p-value")
   kbl(reg.tab, rownames = TRUE, format = "html", digits = 3,
