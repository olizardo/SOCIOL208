...
Recall that in the [one-mode case](prest-eigen.qmd), multiplying the adjacency matrix times its transpose yields the **common neighbors matrix** $\mathbf{M}$:
...
Note that the entries of this matrix are *not* symmetric, like in the standard Breiger projection. For instance, the weight of the link going from Evelyn to Laura is `r round(P.pp[1, 2], 3)`, but the weight of the link going from Laura to Evelyn is `r round(P.pp[2, 1], 3)`.
...
```{r}
   G.dp <- graph_from_adjacency_matrix(P.pp, mode = "directed", weighted = TRUE, diag = FALSE)
   G.dg <- graph_from_adjacency_matrix(P.gg, mode = "directed", weighted = TRUE, diag = FALSE)
```
