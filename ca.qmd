...
Two-mode CA boils down to the eigendecomposition of a suitable matrix, derived from the original affiliation (bi-adjacency) matrix of a two-mode network. So which matrix should be use for CA? 
...
```{r}
   norm.CA.vec <- function(x, s = 0.000001) {
      CA.p <- eigen(diag(1/rowSums(x)) %*% x %*% diag(1/colSums(x)) %*% t(x)) #person CA
      CA.g <- eigen(diag(1/colSums(x)) %*% t(x) %*% diag(1/rowSums(x)) %*% x) #group CA
      ...
      for (i in 1:m) {
         norm.p <- as.numeric(t(CA.p[, i]) %*% diag(rowSums(x)) %*% CA.p[, i]) #person norm
         Z.u.p[, i] <- CA.p[, i] * sqrt(sum(A)/norm.p) #normalizing to unit variance
         Z.v.p[, i] <- Z.u.p[, i] * sqrt(ev[i]) #normalizing to square root of eigenvalue
         norm.g <- as.numeric(t(CA.g[, i]) %*% diag(colSums(x)) %*% CA.g[, i]) #group norm
         Z.u.g[, i] <- CA.g[, i] * sqrt(sum(A)/norm.g) #normalizing to unit variance
...
```{r}
   ...
   p <- p + labs(x = "First Dimension", y = "Second Dimension") + ylim(-1.5, 3) + xlim(-1.5, 1.5)
   p
```
