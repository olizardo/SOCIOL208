...
The two most popular versions of normalized vertex similarity scores are the **Jaccard index** and the **cosine coefficient** (sometimes also referred to as the **Salton Index**). 
...
Where $k$ is the length of the maximum path considered. @lu_etal09 argue that these higher order paths don't matter. Any other issue is that there was already an all-paths similarity measure in existence developed by the mathematical social scientist Leo Katz in the 1950s (!). 
...
```{r}
   katz.sim <- function(A, min.alpha = 0.05) {
      ...
      alpha <- runif(1, min = min.alpha, max = max.alpha)
      S <- solve(I - alpha * A) 
      return(S)
   }
```

For technical reasons we need to choose $\alpha$ to be a number larger than zero but smaller than the reciprocal of the first eigenvalue of the matrix. Hence we just pick a random value in that interval in lines 3 and 4 (set the seed to get reproducible answers). 
...
```{r}
   LHN.sim <- function(A, alpha = 0.9) {
      D <- solve(diag(rowSums(A))) #creating degree matrix
      lambda <- eigen(A)$values[1]
      S <- D %*% solve((alpha * A)/lambda) %*% D
      rownames(S) <- rownames(A)
      colnames(S) <- colnames(A)
      return(S)
   }
