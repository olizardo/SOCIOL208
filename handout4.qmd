...
Let us, therefore, begin with the distance approach. Here the goal is simply to pick a distance metric $d$ with a well defined minimum $d_{min}$ or maximum value $d_{min}$, such that:
...
$$
    ...
    \frac{
    \sum_{i \neq k}
    (a_{ik} - \overline{a}_{i}) \times 
    \sum_{j \neq k}
    (a_{jk} - \overline{a}_{j})
    }
    {
    \sqrt{
    \sum_{i \neq k}
    (a_{ik} - \overline{a}_{i})^2 \times
    \sum_{j \neq k}
    (a_{jk} - \overline{a}_{j})^2
        }
    }
$$

A more involved but still meaningful formula compared of the Euclidean distance. Here $\overline{a}_{i}$ is the *row(column) mean* of the entries of node $i$ in the affiliation matrix. 

So the correlation distance is the ratio of the covariance of the row(column) vectors corresponding to each node in the adjacency matrix and the product of their standard deviations. 
...
The columns of these two matrices are the two blocks we found before. Of course, to implement this method as a *divisive* clustering algorithm, what we want is to split these two blocks into two finer grained blocks, to find our original four structurally equivalent groups. 
...
```{r}
   ...
   ord <- unlist(blocks(g.flint))
   A <- A[ord, ord]
   corrplot(A, is.corr = FALSE, method = 'square', tl.cex = 0.5, 
