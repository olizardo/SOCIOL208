...
Just like as we saw when discussing the [Bonacich Eigenvector centrality in the two-mode case](tm-prest-eigen.qmd), there is a *duality* between the CA scores assigned to the person and the groups on each dimension, such that the scores for each person are a weighted sum of the scores assigned to each group on that dimension and vice versa [@faust97, 171]. 
...
Note the similarity between this blocking and that obtained from the [generalized vertex similarity analysis of the same network](tm-sim.qmd).
