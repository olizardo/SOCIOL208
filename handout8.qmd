...
```{r}
   blau <- function(n, w, a) {
      ...
      p <- 1 - sum((table(vertex_attr(x, a))/vcount(x))^2)
      return(p)
      }
```

Let's see Padme's homeworld ego network diversity score:

```{r}
   round(blau("PADME", g, "homeworld"), 3)
```

Which says that Padme has a fairly diverse ego network when it comes to homeworld. 

We can, of course, just use `sapply` to compute everyone's homeworld ego network diversity score:

```{r}
   H <- sapply(V(g)$name, blau, w = g, a = "homeworld")
   round(H, 3)
```

In this network Count Dooku stands out as having a very diverse ego network by homeworld, while Owen sports a very homogeneous ego network on the same attribute. 
