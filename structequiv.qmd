...
#| fig-cap: "A toy graph demonstrating structural equivalence."
   library(igraph)
   library(ggraph)
   g.toy <- make_empty_graph(9, directed = FALSE) %>% 
...
```{r}
#| echo: false
   A <- as.matrix(as_adjacency_matrix(g.toy))
   library(kableExtra)
...
```{r}
   m <- as.matrix(as_adjacency_matrix(g.toy))
   C <- cor(m)
   round(C, 2)
```
