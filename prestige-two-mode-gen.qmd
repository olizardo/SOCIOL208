...
Note that in this case, "omnivore" elites will be low status, and elites whose affiliations remain tied exclusively to elite events will retain status. The same goes for column objects, as only those events more likely to be attended by more discerning elites (those who completely avoid low status events) will retain status. In formal terms, the status of row objects is heavily contaminated by having non-elite column-object affiliations, which we may refer to as a *row object contamination* system. 
...
#| echo: false
    ...
    kbl(A, format = "pipe") %>% 
    kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), font_size = 10) |> 
