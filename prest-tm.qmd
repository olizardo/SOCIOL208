...
Below is a quick function that plays a two-mode version of the status distribution game that we described in the [status and prestige lesson](prest-eigen.qmd), and which is really just a modification of the one-mode HITS algorithm:
...
Just like we saw in the discussion of [prestige scoring for one-mode networks](prest-eigen.qmd), this is the usual cat chasing its own tail circular equation problem. To know the person status scores $\mathbf{x}$ we need to know the group status scores $\mathbf{y}$ and vice versa.

This is also a classic eigenvector/eigenvaue problem, which means that we can find the person and groups status scores using the [eigendecomposition](prest-eigen.qmd) approach. 
...
@he_etal16 provide a small twist on the double-normalized approach. They suggest that rather than normalizing the adjacency by the inverse of the degrees of each node, we should use the *square root* of the degrees. They call their approach "Birank". 
...
p.tab <- data.frame(
  hits = round(tm.hits$p.s / max(tm.hits$p.s), 3),
...
g.tab <- data.frame(
  hits = round(tm.hits$g.s / max(tm.hits$g.s), 3),
...
kbl(p.tab,
  format = "html", align = "c",
  caption = "Person Prestige Scores Ordered by Degree"
) %>%
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
  column_spec(1, bold = TRUE)
kbl(g.tab,
  format = "html", align = "c",
  caption = "Group Prestige Scores Ordered by Degree"
) %>%
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
  column_spec(1, bold = TRUE)
``` 
