...
Consider @fig-toy. According to the usual eigenvector centrality principle, we expect node 1 to have the highest eigenvector score, as it is connected to other well-connected nodes. 
...
However, imagine instead that the colors in @fig-toy stand for "sellers" (tan nodes) and "buyers" (blue nodes). Accordingly, node 1 is trying to get nodes 2, 3, and 4 (in light blue) to *buy* something from them (like a house). However, each of the light blue nodes has *other* prospective sellers (four each). 
