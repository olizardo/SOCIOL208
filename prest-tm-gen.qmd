...
In the lecture note on [Status and Prestige in Two-Mode Netwoks](prest-tm-gen.qmd) we examine various ways of assigning scores to the row and column objects of the biadjacency matrix to derive status scores for the nodes of a two-mode network. 
...
As [noted before](prest-tm-gen.qmd).qmd), one way to generalize the Bonacich scoring is by substituting some other matrix other than $\mathbf{A}$ like the row-stochastic matrix $\mathbf{P}$ into the above equations and let the process run until convergence. 
...
s1.col <- data.frame(
  n = rownames(s1$p.s)[order(s1$p.s / max(s1$p.s), decreasing = TRUE)],
  s = round(sort(s1$p.s / max(s1$p.s), decreasing = TRUE), 2)
)
s2.col <- data.frame(
  n = rownames(s2$p.s)[order(s2$p.s / max(s2$p.s), decreasing = TRUE)],
  s = round(sort(s2$p.s / max(s2$p.s), decreasing = TRUE), 2)
)
s3.col <- data.frame(
  n = rownames(s3$p.s)[order(s3$p.s / max(s3$p.s), decreasing = TRUE)],
  s = round(sort(s3$p.s / max(s3$p.s), decreasing = TRUE), 2)
)
s4.col <- data.frame(
  n = rownames(s4$p.s)[order(s4$p.s / max(s4$p.s), decreasing = TRUE)],
