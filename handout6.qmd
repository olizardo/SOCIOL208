...
#| fig-width: 10
#| fig-subcap:
#|   - "Law Firm Friendship Network by The Best Three-Community Partition According to the Modularity"
   V(ug)$color <- s$v
   levels(s$v) <- c(1,2,3)
   set.seed(123)
   plot(ug, 
...
```{r}
   comm.res <- cluster_leading_eigen(ug)
```

And examine the `membership` object stored in the results list:

```{r}
   names(comm.res$membership) <- 1:length(comm.res$membership)
   comm.res$membership
```
...
```{r}
   library(kableExtra)
   t <- table(V(g)$status, comm.res$membership)
...
This is just a contingency table listing status on the row (1 = partner) and the three leading eigenvector communities in the columns. 

We find that community one (the most coherent; in orange) is composed of `r round((1 - 3/27)*100)`% of partners. So it is a high status clique. Community three (in green) on the other hand is composed of 100% of associates, a less dense, but completely (low) status homogeneous clique. Finally community two (light blue) is a status-mixed clique, composed of nine partners and seven associates. 
