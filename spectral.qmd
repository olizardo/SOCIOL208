...
First, we generate two separate candidate community assignments for people and groups via spectral clustering from CA:
...
Here, we use the first six eigenvectors. 
...
Given this information we can create a $k \times k$ matrix `e` (where $k$ is the maximum number of community clusters in either the person or group vectors) counting the proportion of edges that go from person community $i$ to group community $j$:
...
@suzuki_wakita09 based on work by @murata09 suggest  using the `e` matrix above to compute the modularity of this uneven person/group assignment according to the following formula:

$$
Q = \sum_{i, j} = \frac{e_{ij}}{e_{i+}}\left(\frac{e_{ij}}{vol(A)} - \frac{e_{i+}e_{+j}}{vol(A)^2}\right)
$$

Where $e_{ij}$ is the fraction of edges connecting people in the $i^{th}$ person-community to groups in the $j^{th}$ event-community, $e_{i+}$ is the fraction of edges originating from person-community $i$ (the corresponding row sum of `e`), $e_{+j}$ is the fraction of edges originating from nodes in group-community $j$ (the corresponding column sum of `e`), and $vol(A)$ is the sum of the entries in the bi-adjacency matrix. 
...
To do this, first let's package the steps we used to find calculate `e` from a partition and find the corresponding $Q$ into a function:
