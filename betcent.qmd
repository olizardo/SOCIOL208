...
Recall that, according to @freeman79, the key conceptual distinction between [closeness](closcent.qmd) and betweenness is between (pun intended) the capacity to reach others quickly (e.g., via the shortest paths) and the capacity to *intermediate* among those same paths. High-betweenness nodes control the flow of information in the network between other pairs of nodes in the network when this information or resource flows via shortest paths.^[The lesson on [random walk centrality concepts](random-cent.qmd) covers variations of centrality that relax the assumption that information or resources flow strictly via shortest paths.]
...
This function computes the pair dependency of the nodes entered as values of the `a` and `b` arguments on the node entered as the value of the `c` argument using the entries stored on the `S` matrix according to equation @eq-brandes2. 
...
Which says that Vincent lies on `r round(pair.dep("Pedestrian", "Young Man", "Vincent"), 2) * 100`% of the paths between the The Young Man and The Pedestrian (four out of eight).
...
As noted in @eq-bet1, Vincent's *betweenness* is just the sum of the pair dependencies of each pair of nodes in the graph on him. We could compute that quantity (not very efficiently) for any node in the graph using the following wrapper over the `pair.dep` function:
