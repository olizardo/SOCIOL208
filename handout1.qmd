...
The plot clearly shows that the *Pulp Fiction* network degree distribution is *skewed* with a small number of actors having a large degree $k \geq 15$ while most others have a small degree $k \leq 5$ indicating inequality of connectivity in the system. 
...
```{r}
   ...
   library(here)
   g.el <- g.el[, c("name1", "name2")]
   write.csv(g.el, here("handouts", "pulp.csv"))
```
...
```{r}
#| output: true
   g.el <- read.csv(here("handouts", "pulp.csv"), 
