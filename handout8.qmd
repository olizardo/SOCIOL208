...
Where $k_i$ is the ego network size (ego's degree) and $\bar{k}_{j \in N(i)}$ is the **mean degree** of the subgraph formed by ego's alters (excluding ego). 

So to calculate $ES$ in our network all we have to do is calculate the mean degree of each ego graph and the subtract that from ego's degree. We can obtain the mean degree of each ego using the first two `lapply` and `sapply` lines in the following:
...
The $r^{pb}$ works just like a correlation coefficient. A value 1.0 indicates maximum preference for same category alters, and a value of -1.0 indicates preference to connect with people different from ego. 
...
In this case, Yule's $Q$ is a good option [@borgatti_etal24, p. 159]. 
...
$Q$ works like a correlation coefficient, and it is 1.0 when ego's network is either completely homophilous ($b=0$) or there are no other people in the network of the same category as ego ($c = 0$) and it is -1.0 when ego's network is either completely heterophilous ($a = 0$) or there are no people in the network of a different category from ego ($d = 0$). 
