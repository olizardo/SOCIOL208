...
A more interesting property of the modularity matrix is that we can use to compute the modularity of a given binary partition. Let's take status in the `law_friends` network as an example. 
...
Neat! It makes sense (given the name) that we can compute the modularity from $\mathbf{B}$.
...
Of course, if you are working on your project, you don't have to mess with all of the functions above, because `igraph` has implemented Newman's leading eigenvector (of the modularity matrix) community detection method [@newman06] in a single function called `cluster_leading_eigen`. 
