...
$$
 x^a_i = \sum_j a_{ji} x^h_j
$$

Which says that the hub score $x^h$ of a node is the sum of the authority scores $x^a$ of the nodes they point to (sum over $j$; the outdegree), and the authority score of a node is the sum of the hub scores of the nodes that point to it (sum over $j$; the indegree). 

So we need to make our [status game](prest-eigen.qmd) a bit more complicated (but not too much) to account for this duality:
...
Everything is like [our previous](prest-eigen.qmd) `status1` function except now we are keeping track of two mutually defining scores `a` and `h`. We first initialize the authority scores by setting them to the value of $1/n$ (where $n$ is the number of nodes or the number of rows in the adjacency matrix) in line 2. We then initialize the $\delta$ difference and $k$ counter in lines 3-4. The `while` loop in lines 5-13 then updates the hub scores (to be the sum of the authority scores of each out-neighbor) in line 7, normalizes them in line 8, and updates the new authority scores to be the sum (across each in-neighbor) of these new hub scores, which are then themselves normalized in line 10. 
...
If these nodes were papers, then we would say that both 2 and 11 point to a common third node 1. In an information network, the papers that other papers point to are their common references. Therefore the number of *common out-neighbors* of two nodes is called the **bibliographic coupling** score between the two papers. In the same way, we can see that 2 and 11 are pointed to by a common third neighbor 21. The number of *common in-neighbors* between two-papers is called their **co-citation** score.
...
Recall that [PageRank](prest-pagerank.qmd) works on the $\mathbf{P}$ matrix, which is defined like this:
...
Where $\mathbf{D}_{in}^{-1}$ is a matrix containing the inverse of the indegrees along the diagonals (and zeroes elsewhere) and $\mathbf{A}^T$ is the transpose of the adjacency matrix. Each non-zero entry of $\mathbf{P}_{h}$ is thus equal to one divided by that row node's indegree. 
...
Like the $\mathbf{P}_{a}$ matrix, the $\mathbf{P}_{h}$ matrix is row-stochastic, meaning its rows sum to 1.0:
...
Each of these matrices is row stochastic:
...
@saerens_fouss05 argue that there is an intimate relationship between the HITS dual ranking scores and one of the most widely used multivariate analysis techniques, Principal Components Analysis (PCA). In fact, they argue that HITS is just PCA on an *uncentered data matrix* (which in this case is the network's adjacency matrix). Technically we could also just argue that PCA is just HITS on a **centered adjacency matrix**, as we will see right now.

Let's see how this works. First, let's create a *centered* version of the network adjacency matrix. To center a typical data matrix (e.g., of individuals in the rows by variables in the columns) we subtract the *column mean* (the average score of all individuals on that variable) from each individual's score.^[Note that the column mean of the adjacency matrix will be equivalent to each individual's **indegree** (the sum of the columns) divided by the total number of nodes in the network ($k^{in}/n$), which is kind of a normalized degree centrality score, except the usual normalization is to divide by $n-1$ (so as to have a maximum of 1.0) as we saw in our discussion of [degree centrality](cent-deg.qmd).] 
...
Recall that the covariance between two vectors $x$ and $y$ is just the sum of the values in the vector composed by multiplying the first vector (minus its mean) by the second vector (minus its mean) divided by the length of the vector minus one. 

In the network adjacency matrix case, the vectors in question are the *columns* of the adjacency matrix for each individual, recording each of the incoming ties to that person. So if $\mathbf{a}_i$ is the column vector recording the in-neighbors of $i$ and $\mathbf{a}_j$ is the column vector recording the in-neighbors of $j$, the covariance between nodes $i$ and $j$ is given by:

$$
\begin{equation}
cov(i,j) = \frac{(\mathbf{a}_i - \mathbf{\bar{a}}_i) \cdot (\mathbf{a}_j - \mathbf{\bar{a}}_j)}{N-1}
\end{equation}
$$
...
These scores should be equivalent to the normalized authority scores we computed earlier (because they are based on the indegree). Now to compute the hub scores from the PCA analysis, all we need to do is add up the authority score of each node that a given node points to. We can do this by post-multiplying the adjacency matrix times the PCA score vector:
...
Recall from the previous that the [status game](prest-eigen.qmd) computes the **leading eigenvector** of the matrix we play the game on, and spits that vector out as our status scores for that matrix. The leading eigenvector is that associated with the largest eigenvalue (if the matrix contains one).
...
But @fouss_etal04 note, like we did at the beginning, that a [matrix has many eigenvector/eigenvalue pairs](prest-eigen.qmd) and that perhaps the *second* leading eigenvector is not that useless; this is the eigenvector associated with the *second* largest eigenvalue.
...
In line 2, we store the CA results in the object `ca.res`. We then grab the CA scores associated with the rows of the adjacency matrix and put them in the object `ca.h` in line 3, and the scores associated with the columns of the adjacency matrix and put them in the object `ca.a` in line 4.
...
But what are the CA (and PCA) scores supposed to capture? Let's look at them side-by-side next to the SALSA hub and authority scores, as shown in the table which has nodes rank ordered by their SALSA hub score.
...
Using information in the table, the figure below shows a point and line network plot but using the CA hub and authority scores to embed the nodes in a common two-dimensional space. In the plot, nodes are colored by the *difference* between their SALSA hub and authority scores such that nodes where the hub score is larger than the authority score appear in blue, and nodes where the authority score is larger than the hub score appear in red. 
...
cent.dat <- apply(cent.dat, 2, function(x) {
  round(x / max(x), 3)
})
...
kbl(cent.dat1,
  ...
  caption = "Top Prestige Scores Ordered by Indegree."
) %>%
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
  column_spec(1, bold = TRUE)
...
kbl(cent.dat2,
  format = "html", align = "c", row.names = FALSE,
  caption = "Top Prestige Scores Ordered by Outdegree"
) %>%
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
  column_spec(1, bold = TRUE)
```
