...
In the lecture note on [Status and Prestige in Two-Mode Netwoks](tm-prest-eigen.qmd) we examine various ways of assigning scores to the row and column objects of the biadjacency matrix to derive status scores for the nodes of a two-mode network. 
...
As [noted before](tm-prest-eigen.qmd).qmd), one way to generalize the Bonacich scoring is by substituting some other matrix other than $\mathbf{A}$ like the row-stochastic matrix $\mathbf{P}$ into the above equations and let the process run until convergence. 
