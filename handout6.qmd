...
The `delta.Q` vector stores the results of our modularity checks. In this case, the test in the first community failed; the modularity got worse (difference between old and new is below zero) when we tried to split into two smaller communities. This is usually indicative that the original community was a well-defined **group** with lots of within-cluster ties so that splitting it makes everything worse (that's the orange nodes in the figure above).
