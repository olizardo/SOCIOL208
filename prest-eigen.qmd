...
Under this interpretation, actors gain status and prestige in the network by being connected to prestigious, high-status others. Those others, in turn, get their status from being connected to high-status others, and so on *ad infinitum*. 
...
Note that all this obscure talk about eigenvalues and eigenvectors is just matrix linear algebra. It has nothing to do with networks and social structure.
...
The undirected friendship network is shown in .
...
Consider . According to the usual eigenvector centrality principle, we expect node 1 to have the highest eigenvector score, as it is connected to other well-connected nodes. 
...
However, imagine instead that the colors in stand for "sellers" (tan nodes) and "buyers" (blue nodes). Accordingly, node 1 is trying to get nodes 2, 3, and 4 (in light blue) to *buy* something from them (like a house). However, each of the light blue nodes has *other* prospective sellers (four each). 
