...
Which suggests that the three-cluster partition of the network (shown in -2) does pretty well in separating densely connected subgraphs ($Q = 0.36$). 
...
We can see that the three-cluster solution does really well modularity-wise ($Q = 0.37$), however, the four cluster solution also seems promising. The resulting communities are shown in 3-1 and 3-2. 
