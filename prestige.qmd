...
Here's a function that computes this matrix setting $\alpha$ to a number that is *almost* equal to the first eigenvalue of the adjacency matrix (e.g., that number minus a very teeny-tiny number: $10^{-10}$):

```{r}
   katz.sim <- function(w) {
      I <- diag(nrow(w)) #creating identity matrix
      alpha <- 1/eigen(w)$values[1] - 1e-10 #first eigenvalue of adjacency matrix minus a tiny number
