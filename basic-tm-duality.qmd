...
As we just saw, projecting the original biadjacency matrix using Breiger's [-@breiger74] approach results in two weighted one-mode networks. Because there is an edge between two persons (groups) if they share at least one person (group), the resulting graphs tend to be dense, with high connectivity between nodes, as in . This can make it hard to discern the connectivity structure of the projected networks and detect patterns.
