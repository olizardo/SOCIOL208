...
The resulting scores are the same ones returned by the "co-HITS" algorithm of @deng_etal09. On this ranking, Nora, Theresa, and Evelyn are tied at the top and Olivia and Flora are tied at the bottom. The top ranked event is the one that took place on 9/16. 
...
Now to compute our double-normalized PageRank status scores, we just play the status game on the new matrices:

```{r}
   tm.status(P.n2, G.n2)
```

Which implements the "BGRM" algorithm of @rui_etal07. We can see that the most highly ranked women are now Olivia and Flora (in contrast to Evelyn, Theresa, and Nora in the standard PageRank scores) and the top event is the one held on 2/23. 
