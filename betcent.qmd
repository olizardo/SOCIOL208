...
Recall that the key conceptual distinction between closeness and betweenness according to @freeman79 is that between (pun intended) the capacity to reach others quickly (e.g., via the shortest paths) and the capacity to *intermediate* among those same paths. High betweenness nodes control the flow of information in the network between other nodes.
...
We can define the **pair dependency** $\delta_{i(k)j}$ of a pair of nodes $\{i, j\}$ on another node $k$ as follows:
...
<!-- Interestingly, @brandes01 shows that the dependence of a node $i$ on another node $k$ can also be expressed as the ratio of the number of shortest paths between $i$ and $k$ divided by the number of shortest paths between $k$ and other nodes $j$ where $j$ is a neighbor of $k$ multiplied by the dependence of $k$ and $j$ plus one, summed over all of $k$'s neighbors $j$.

$$
\delta_{ik}\sum_j = \frac{\sigma_{ik}}{\sigma_{kj}} (\delta_{kj} + 1)
$$ {#eq-brandes}

The betweenness of a node $k$ and in turn be expressed in terms of the last equation as folows:

$$
C^{BET}_k = \sum_i \delta_{ik}
$$ {#eq-bet2} -->
...
The function has three arguments: The graph (`g`), the `from` node (the starting node in the path count) and the `to` node (the end node in the path count), storing the results in a list object containing various sub-objects. The actual paths are in a list sub-object called `vpaths`, which is itself a list (with named vectors containing the indices of the nodes of each path as elements). The `length` of this list is the number of shortest paths between the waitress and the pedestrian:
...
Which provides us with the relevant entry in the geodesic distance matrix, telling us that the young man is four steps away from the pedestrian in the network---meaning that each path will involve five nodes (counting them, with three intermediary nodes).

So the denominator for equation @eq-pair is the length of `ap$vpaths`:
...
```{r}
    num <- sapply(ap$vpaths, function(x) {"Vincent" %in% names(x)})
    num <- sum(num)
    num
```

The statement inside the function just checks whether the character string "Vincent" is present in the named vector of node labels in each path stored in `vpaths`. The sum of the (one/zero or logical TRUE/FALSE) entries of this vector is the number of times Vincent is in a path linking The Pedestrian and The Young Man. 
...
As noted, Vincent's *betweenness* is just the sum of the dependencies of each pair of nodes in the graph on him. We could compute that (not very efficiently) as follows:
...
We can of course use the same function to (efficiently) compute the betweenness centrality of each node in the graph as store it in a vector of length $N$:
