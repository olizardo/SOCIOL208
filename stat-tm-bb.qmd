...
Recall from our lecture on [the duality of persons and groups](basic-tm-duality.qmd) that a common approach to analyzing two-mode networks is via what @everett_borgatti13 call the "dual projection" approach. This involves computing the row and column object **projections** from the original affiliation matrix [@breiger74]. Nevertheless, one issue with this approach is that the result is a weighted network, which can be hard to analyze using our usual tools. 
...
Now, our job is to binarize these matrices by retaining adjacency relations between nodes whose weights is significantly larger than we would expect given a suitable null model. This is a situation perfectly tailored for [two-mode graph ensembles](stat-tm.qmd)!
...
```{r}
P.list <- lapply(B, function(x) {
  x %*% t(x)
}) # ensemble of row projections
```

And a bunch of group projections:

```{r}
G.list <- lapply(B, function(x) {
  t(x) %*% x
}) # ensemble of column projections
```
...
```{r}
P.bin <- lapply(P.list, function(x) {
  (P > x) * 1
})
G.bin <- lapply(G.list, function(x) {
  (G > x) * 1
})
```
