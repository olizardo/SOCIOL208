...
Note the normalization step on line 9, which is necessary to prevent the sum of status scores from getting bigger and bigger indefinitely. In base `R`, the `type = "E"` normalization implements the **euclidean vector norm**, by which we divide each value of the status scores by after each update.^[For a vector of numbers $\mathbf{x}$ the euclidean vector norm $||\mathbf{x}||_2$ is given by: $\sqrt{\sum x^2}$.]
...
Everything is like our previous `status1` function except now we are keeping track of two mutually defining scores `a` and `h`. As you may have guessed this is just an implementation of the "HITS" algorithm developed by @kleinberg99.
...
#| echo: false
   nodes <- 1:vcount(g)
   pr.cent <- round(status1(t(G))/max(status1(t(G))), 3)
   hu.cent <- round(h/max(h), 3)
   au.cent <- round(a/max(a), 3)
   pr.dat <- data.frame(Nodes = nodes, PR.Cent = pr.cent, ODeg.Cent = degree(g, mode = "out"), IDeg.Cent = degree(g, mode = "in"))
   hu.dat <- data.frame(Nodes = nodes, Hub.Cent = hu.cent, ODeg.Cent = degree(g, mode = "out"), IDeg.Cent = degree(g, mode = "in"))
   au.dat <- data.frame(Nodes = nodes, Auth.Cent = au.cent, ODeg.Cent = degree(g, mode = "out"), IDeg.Cent = degree(g, mode = "in"))  
   pr.dat <- pr.dat[order(pr.dat$PR.Cent, decreasing = TRUE), ]
   hu.dat <- hu.dat[order(hu.dat$Hub.Cent, decreasing = TRUE), ]
   au.dat <- au.dat[order(au.dat$Auth.Cent, decreasing = TRUE), ]

   kbl(pr.dat[1:5, ], 
       format = "html", align = "c", row.names = FALSE,
       caption = "Top Five PageRank Scores.") %>%    kable_styling(bootstrap_options = c("hover", "condensed", "responsive"))
   
   kbl(hu.dat[1:5, ], 
       format = "html", align = "c", row.names = FALSE,
       caption = "Top Five Hub Scores.") %>%    kable_styling(bootstrap_options = c("hover", "condensed", "responsive"))
   
   kbl(au.dat[1:5, ], 
       format = "html", align = "c", row.names = FALSE,
       caption = "Top Five Authority Scores.") %>%    kable_styling(bootstrap_options = c("hover", "condensed", "responsive"))
```
