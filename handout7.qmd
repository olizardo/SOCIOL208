...
## PageRank Status for Two Mode Networks
...
For the people, we can do this by taking the original affiliation matrix, and pre-multiplying it by a **diagonal square matrix** $\mathbf{D}_P^{-1}$ of dimensions $M \times M$ containing the *inverse* of the degrees of each person in affiliation network along the diagonals and zeros everywhere else, yielding the row-stochastic matrix $\mathbf{P}_P$ of dimensions $M \times N$:
...
Which produces the matrix $\mathbf{P}_{PP}$ a square $M \times M$ matrix containing the *degree-normalized similarities* between each person.
...
Which produces the matrix $\mathbf{P}_{GG}$ a square $N \times N$ matrix containing the *degree-normalized similarities* between each group.
...
**Correspondence Analysis** (CA) a relatively simple way to analyze and visualize two-mode data. In fact, we have already computed most of what we need to perform a CA of the two-mode network when figuring out the PageRank Status scores. 

First, let us review the idea of an **eigendecomposition** of a square matrix. Let's say we have the following matrix $\mathbf{B}$:
...
Most matrices like this can be decomposed into two other matrices $\mathbf{U}$ and $\mathbf{\lambda}$, such that the following equation is true:
...
In `R` you can find the values that yield the eigendecomposition of any matrix (if one exists) using the function `eigen` (along with the `Re` function to return real, not imaginary numbers). So in our case this would be:
...
Now, the idea is that we can do this with any matrix, including a network adjacency matrix or a proximity matrix derived from it like the ones we used to calculate the PageRank Status scores earlier. Choosing the first few columns of the result $\mathbf{U}$ then gives us the best low dimensional approximation of the original matrix. 

CA is based on this principle, and it works on the matrices $\mathbf{P_{PP}}$ and $\mathbf{P_{GG}}$ as we defined them earlier:
...
So this is interesting. The first eigenvector of the decomposition of both $\mathbf{P_{PP}}$ and $\mathbf{P_{GG}}$ is just the same number for each person and group. Note that this is the eigenvector that is associated with the first eigenvalue which happens to be $\lambda = 1.0$.
...
```{r}
   ...
   p <- p + scale_x_discrete(position = "top") 
   p <- p + geom_hline(yintercept = 8.5, linewidth = 2, color = "blue")
   p <- p + geom_hline(yintercept = 10.5, linewidth = 2, color = "blue")
   p <- p + geom_vline(xintercept = 9.5, linewidth = 2, color = "blue")
   p <- p + geom_vline(xintercept = 6.5, linewidth = 2, color = "blue")
   p
...
Here CA seems to have detected to cluster of actors who preferentially attend two distinct clusters of events! The events in the middle don't seem to differentiate between participants in each cluster (everyone attends), but the events in the lower left and upper right are attended by a distinct cluster of participants. 
