...
If the above equality obtains, then the number of shortest paths between $i$ and $j$ that involve $k$ as an inner node (the numerator of air) is simply $s_{ik} \times s_{kj}$. This means that the pair dependency of $i$ and $j$ on $k$ will be just:
