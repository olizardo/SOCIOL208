...
Neat! These are the same scores we obtained by playing our status game.  
...
```{r}
   weight <- 1/sqrt(eig.p$values[1])
   weight * sum(A["EVELYN", ] * g.s)
```
...
### Performing CA and Visualizing Two Mode Networks
