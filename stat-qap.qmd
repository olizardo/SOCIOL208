...
So, in contrast to the [edge swapping approach](stat-swap.qmd), which preserves *some* graph characteristics but loses others, in the permutation approach, *every* graph-level characteristic stays the same:
...
```{r}
p.vals <- c(
  1 - ecdf(abs(betas[, 1]))(reg.res$coefficients[1]),
...
kbl(reg.tab[c(2:3), ],
  rownames = TRUE, format = "html", digits = 3,
