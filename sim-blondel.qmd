---
title: "Structural Similarity Across Graphs"
---
...
blondel.sim <- function(a, b) {
  ...
  z <- apply(z, 2, function(x) {
    x / max(x)
  }) # normalizing converged matrix by column max
  return(z) # return converged similarity matrix
}
```

Which is modeled after our status game function but instead of computing a vector of scores we are populating a whole matrix! For instance, it is 
...
In which case, the resulting "similarity" scores, will be equivalent to Kleinberg's [Hub and Authority scores](prest-hits.qmd)! 
...
K <- blondel.sim(A, B)
tab <- cbind(K,
  Hub.Score = round(hits_scores(g2)$hub, 4),
