...
Now that we know how to get the proportions we need, we can write a custom function that will compute $H$ for a given ego network for any given attribute:

```{r}
   blau <- function(n, w, a) {
      x <- subgraph(w, neighbors(w, n))
      H <- 1 - sum((table(vertex_attr(x, a))/vcount(x))^2)
      return(H)
      }
```

This function takes three inputs: The name of the ego `n`, the graph object `w`, and the name of the attribute `a`. It returns the Blau diversity index score for that ego on that attribute. 
...
Diversity measures the extent to which ego's connect to diverse alters. We may also want to get a sense of how *homophilous* an ego network is, namely, the extent to which ego connects to alters that are the same or different from *them*.

Note that these are two different things. A person can have an ego network composed of 100% alters who are different from them on a given attribute (maximum "heterophily") but those alters could be 100% homogeneous---e.g., all come from the same planet---and thus ego will have the minimum Blau diversity score ($H = 0$).  
...
As we can see, the Emperor, Mace Windu, Obi-Wan and other such characters have a homophilous "bro" network. Padme on the other hand has a heterophilous network. 
...
As we can see, Mace Windu is surrounded by other men (like him) but Padme's network includes only one other woman, and the rest are composed of people with a different gender presentation than her.
...
Which shows a slight homophily preference. 
...
Here we can see that the reason why R2-D2 ends up being high in homophily in the weighted scale despite containing a network with just three nodes, is that he is connected to C3PO who is the only other character whose gender is assigned to "none."

And here's the graph's WEI:
...
Which shows that our previous unweighted average under-estimated homophily in this network.
