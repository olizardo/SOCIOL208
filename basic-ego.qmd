...
Here we see the problem is that both Greedo and Jabba are pendant nodes (with degree equal to one), so it doesn't make sense to analyze their clustering coefficients because their ego graph is just an isolated node!
...
Note that in this analysis, Luke has the lowest clustering coefficient ($C = 0.34$) this usually indicates an ego whose alters are partitioned into distinct clusters (and hence they are not connected to one another), and ego is a mediator or *broker* between those clusters [@burt92].
...
If we have information on the categorical attributes of ego's alters we may be interested in how diverse are ego's choices across those attributes. 
...
As we can see, Ki-Adi-Mundi and Captain Typho have an ego network that combine maximum homophily and homogeneity with respect to gender (both ego networks are composed of all men)--and therefore both receive the minimum score of $Q=-1$.
...
Which also suggests a slight same-gender preference in the network after accounting for unbalanced group sizes. 
