...
@tbl-fclc shows the linear fitness/complexity scores compared to the non-linear ones for persons and groups, and @fig-sw-3 shows the biadjacency matrix with rows and columns re-orodered according to the linearized fitness/complexity scores, which we can see is pretty close to the non-linear version in @fig-sw-2.
