...
$$
C_B(Ego) = \frac{1}{2}\sum_{i \neq j}\left[\mathbf{A}^2 \otimes (\mathbf{J} - \mathbf{A})\right]_{ij}^{-1}
$$

Where $\mathbf{J}$ is a matrix full of ones of the same dimensions as $\mathbf{A}$ and $\otimes$ indicates element-wise matrix multiplication.
