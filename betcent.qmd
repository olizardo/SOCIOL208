...
As noted in @eq-one, the *one-sided dependencies* of a node $i$ on another node $j$ is just the sum of these pairwise dependencies involving $i$ and $k$ across every node $j$. We can get these using a simple wrapper function oer `pair.dep`:
...
```{r}
    one.sided <- function(a, c) {
        ...
        k <- which(V(g)$name == c)
        for (j in 1:n) {
            if (i != j & k != j) {od <- od + pair.dep(i, j, k)}
            }
        return(od)
        }
```
