...
Here's a function called `tm.fitness` that modifies the old two-mode status distribution game function [we played before](tm-prestige.qmd) to compute the fitness and complexity prestige scores for persons and groups:
...
#| tbl-cap: "Status Scores for Persons"
   ...
   kbl(s.tab, format = "pipe", digits = 3,
       align = c("l", "c", "c"),
       col.names = c("Person", "Bonacich", "Fitness")) %>% 
...
#| tbl-cap: "Status Scores for Groups."
   ...
   kbl(s.tab, format = "pipe", digits = 3,
       align = c("l", "c", "c"),
       col.names = c("Group", "Bonacich", "Complexity")) %>% 
   kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
   column_spec(1, bold = TRUE) |> 
   footnote(symbol = c("Scores normalized by dividing by the maximum.")) 
```
...
To help us make sense of the differences between the Bonacich and the fitness/complexity prestige scoring, the table below shows the original SW biadjacency matrix with rows and columns ordered by the degree of each node. This means that highly attended events appear in on the left, and highly active women appear on the top.  
...
#| echo: false
    library(kableExtra)
    A.tab <- A[names(sort(rowSums(A), decreasing = TRUE)), names(sort(colSums(A), decreasing = TRUE))]
    kbl(A.tab, format = "pipe") %>% 
...
The table gives us some insight as to why event *\{9/16\}* drops so much in the fitness/complexity rankings. While it is the *best* attended event (leftmost in @tbl-sw) this also means that it is attended by a lot of low status women (ones at the bottom of the table). Event *\{3/2\}* on the other hand, while having relatively sparse attendance (only three women) has a membership composed of exclusively hight status persons (ones at the top of the table). Event  *\{2/23\}* gets ranked similarly by the two measures because it has both sparse attendance and the majority (two thirds) of its attendees are low status women. 

@tbl-sw also helps us make sense of the *\{Theresa\}* versus *\{Evelyn\}* contrasts across prestige scores. Both are highly active women, but Evelyn's membership in the top two events according to fitness/complexity *\{3/2, 6/27\}*  puts her towards the top. The same goes for Nora's membership in *\{11/21, 8/3\}*, which improves her standing in the fitness/complexity ranking. 
...
Here are the person scores:

```{r}
#| echo: false
#| label: tbl-fcp
#| tbl-cap: "Fitness Scores for Persons"
   library(kableExtra)
   library(dplyr)
   ...
   kbl(s.tab, format = "pipe", digits = 3,
       align = c("l", "c", "c"),
       col.names = c("Person", "Fitness", "Linear Fitness")) %>% 
   kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
   column_spec(1, bold = TRUE) |> 
   footnote(symbol = c("Scores normalized by dividing by the maximum.")) 
```
...
#| echo: false
#| label: tbl-fcg
#| tbl-cap: "Complexity Scores for Groups"
   library(kableExtra)
   library(dplyr)
   nfc.gs <- fc$g.s * k.g
   ...
   kbl(s.tab, format = "pipe", digits = 3,
       align = c("l", "c", "c"),
       col.names = c("Person", "Complexity", "Linear Complexity")) %>% 
   kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
   column_spec(1, bold = TRUE) |> 
   footnote(symbol = c("Scores normalized by dividing by the maximum.")) 
```