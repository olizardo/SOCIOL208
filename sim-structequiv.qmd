...
kbl(Equiv, align = "c", format = "html") %>%
  ...
  column_spec(c(4, 6, 8), extra_css = "border-right: 3px solid;")
```

Once the equivalence matrix is re-ordered we can see that sets of structurally equivalent nodes in , appear clustered along the diagonals. This type of re-arranged matrix is said to be in **block-diagonal** form (e.g., non-zero entries clustered along the diagonals).
...
blocks <- function(w, s = 2) {
  if (is.null(colnames(w)) == TRUE) {
    colnames(w) <- paste("v", 1:ncol(w), sep = "_")
  } # naming columns if they have no name
  z <- list(cor(w))
  for (i in 1:s) {
    z <- unlist(lapply(z, con.cor), recursive = FALSE)
  }
