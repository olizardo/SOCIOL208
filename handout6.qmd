...
Let's move to a real example. Below are two plots of the advice network from the `law_advice` data from the `networkdata` package. The first shows the nodes colored by status in the firm (partners versus associates) and the other shows the nodes colored by gender. It is pretty evident that there is more community by status than by gender. So a measure of modularity should be higher when using status as our partition than when using gender.

```{r}
#| fig-height: 10
#| fig-width: 10
#| fig-subcap:
#|   - "Law Advice Friendship Network by Status"
#|   - "Law Advice Friendship Network by Gender"
#| echo: false
   library(networkdata)
   library(igraph)
set.seed(123)
...
Note than an edge list is a "bag of links" so we can compute the three probabilities we need to compute the modularity of a partition directly from the edge list data frame. Here's a function that does this:
...
```{r}
   link.dat1 <- link.bag(g, V(g)$status)
   link.dat2 <- link.bag(g, V(g)$gender)
   mod.Q1(link.dat1)
   mod.Q1(link.dat2)
```
...
Note that while the "bag of links" idea is good to show the basic probabilistic principle behind the modularity in a network we can compute directly from the adjacency matrix without going through the edge list data frame construction step. A function that does this looks like:
