...
Putting these three lessons together suggests a simple way to the multiple community detection in a network using the eigenvalues of a suitable matrix (along with standard clustering algorithms) that doesn't rely on the iteration of a divisive approach across different splits that only use one (the leading) eigenvector. This general approach to community detection is sometimes referred to as **spectral clustering**.^[Not as a reference to ghosts, but because the eigendecomposition of a matrix is sometimes also called the **spectral decomposition**.]
...
We then row-normalize each of these values (e.g., using the Eucledian norm) and use the $\mathbf{U}$ vector as input to a kmeans algorithm with known number of clusters [@fouss_etal16, p. 320]. 
...
This function takes $\mathbf{U}$ matrix as input and returns a nine-element list (with vectors of length $n$ as its elements) of cluster assignments for each node in the graph. Each vector corresponds to partitions $k = \{2, 3, \ldots 10\}$ respectively. 
...
We saw how to cluster a network in a way that results in a good community partition using the Laplacian of the adjacency matrix $\mathbf{L}$. Another approach is use a (degree) normalized version of the same matrix $\mathbf{\hat{L}}$, defined as follows:
...
Now let's see if the normalized cut helps finds some communities:
