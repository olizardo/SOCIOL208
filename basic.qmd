...
- First (line 1), we sum all the rows (or columns) of the geodesic distance matrix. This vector (of length equal to the number of nodes) gives us the sum of the geodesic distances from each node to every other node (we will use this to compute [**closeness centrality**](cent-clos.qmd) later). 
