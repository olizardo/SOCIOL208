...
In the case of the adjacency matrix, we proceed in the same way, computing the covariance matrix of the columns of the adjacency matrix (e.g., the covariance between each node's indegree). We can use the native `R` function `cov` for this purpose:
