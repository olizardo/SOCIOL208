...
So it seems like this, the minimum time it takes for something that starts with me to get to you [@fouss_etal04], is a good measure of the proximity between me and you in the graph. 
...
We can of course, write a function to compute the average first passage time from one node to another using every node in the graph as the initial seed:
...
The accuracy of the average first pass estimate we get depends on the number of replications we use to get the average. The bigger, the more accurate. However, it can take a lot of computing time if we increased `n` to a giant number.

Thankfully there is a way to use matrix algebra magic to compute the average first pass at the limit ($n \approx \infty$).
