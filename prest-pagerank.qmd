...
D.o <- diag(ifelse(out_degree == 0, 0, 1 / out_degree))
```
...
Now we can just adapt the the model of status distribution we used for [eigenvector centrality](prest-eigen.qmd) but this time using the $\mathbf{P}$ rather than the $\mathbf{A}$ matrix. Note that because we are interested in the status that comes **into** each node we use the *transpose* of $\mathbf{P}$ rather than $\mathbf{P}$, just like we did for the @bonacich72 status score. 
...
```{r}
pr_scores <- page_rank(g, damping = 1, algo = "arpack")$vector
round(pr_scores / max(pr_scores), 3)
```

So the distributional model of status is the same one implemented in the PageRank algorithm! 

## PageRank as a Markov Diffusion Model
...
```{r}
#| label: fig-krack
#| fig-cap: "Krackhardt's managers directed advice network."
#| fig-width: 8
#| fig-height: 8
#| echo: false
set.seed(123)
...
```{r}
pr_scores <- page_rank(g, damping = 0.85, algo = "arpack")$vector
round(pr_scores / max(pr_scores), 3)
```
