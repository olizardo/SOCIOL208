...
A **rewiring** of this graph, which preserves these statistics, goes like this: Pick a random edge, detach it from the vertices that are incident to it, and attach it to a new pair of vertices that are current disconnected (no multiples), making sure they are distinct vertices (no loops). Do that for some proportion of the edges in the graph. 
...
## Correlating Ties Across Two Networks
...
We can use the same approach as before to answer this question, since a bivariate correlation between ties between two networks is just like the univariate statistics we computed earlier (in that case the modularity, but it could been any network level statistic like the diameter or centralization).

Let's see an example. Suppose we have a hypothesis that the relation of "friendship" should be correlated to that of "adivice"; that is, *people are more likely to seek advice from friends*. 

The Krackhardt High-Tech Managers data contains information on both the advice and friendship relations between the actors. Let's load up the advice network:
...
Which is positive, suggesting that people do tend to seek advice from friends. However, is this correlation larger than we would expect by chance?

```{r}
   perm <- function(x) {
      A <- as.matrix(as_adjacency_matrix(x))
      s <- sample(1:nrow(A))
      A <- A[s, s] #permuting matrix
...
Here's a function for the correlation:
...
```{r}
   ...
   p <- p + geom_histogram(binwidth = 0.02, stat = "bin", fill = "darkblue")
   ...
   p <- p + theme_minimal() + labs(x = "Q by Level", y = "Freq.")
   p <- p + theme(axis.text = element_text(size = 12))
   p <- p + annotate("text", x=-0.04, y=100, label= "Zero Point", color = "purple")
   p <- p + annotate("text", x=0.22, y=100, label= "Obs. Value", color = "red")
   p
```
