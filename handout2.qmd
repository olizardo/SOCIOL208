...
Now we have a `list` object in `R` of length equal to the number of younger women with each entry equal to the ids of those women's in-neighbors. Now we need to create a vector of the unique ids of these nodes:
...
Now we are ready to create our subgraph! All we need to do is specify we want *both* the younger law firm women and their in-neighbors:

```{r}
   nodes <- unique(c(yw, yw_in))
   g <- subgraph(law_advice, nodes)
```

We can even specify a new vertex attribute, differentiating the focal network from the in-neighbor network.

```{r}
   V(g)$net_status <- c(rep(1, length(yw)), rep(2, length(nodes) - length(yw)))
   vertex_attr(g)
```
