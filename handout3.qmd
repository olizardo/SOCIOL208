...
So the idea is to combine the HITS reflective centrality idea with PageRank's degree-weighting scheme. We can do this by using a variation of the status game we used for HITS, developed by @lempel_moran01.^[They called their approach SALSA an acronym for the also impossible to remember title of "Stochastic Approach for Link Structure Analysis."] Instead of using them the original adjacency matrix $\mathbf{A}$ we use a couple of degree weighted variations:
