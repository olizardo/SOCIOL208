...
So one approach is to set $\gamma = 1$ above so that a product's status is the sum of the inverse of the inverse of the status of the countries that produce it. This means that a product's status decreases non-linearly when it is produced by low status countries. This feeds back into the calculation of a country's status, such that producing low status products "contaminates" the status of a given country driving it downwards. 

We can of course apply the same reasoning to a country's status and set $\delta = 1$, which models a system in which a country's status decreases non-linearly when it produces low status products. This feeds back into the calculation of a product's status, such that a product produced by a low status country is downgraded in a stronger way. 

A system in which *both* countries and product status decrease non-linearly when they are affiliated with low-status objects (double status contammination, feeding back into one another), would have *both* $\gamma = 1$ and  $\delta = 1$.
...
#| tbl-cap: "Status Scores for Groups"
   ...
   kable_styling(bootstrap_options = c("hover", "condensed", "responsive"))
```
And the original biadjacency matrix is:
