...
Note that if you collect traditional ("whole") network data (e.g., all the people in a classroom) you have ego network data for that setting (the ego subgraphs of each node as just defined), but not the reverse; ego-network data cannot be converted into "whole" network data, although it can approximate it. 
...
The `starwars` object contains a `list` of igraph network data sets corresponding to seven of the Star Wars films (the three originals, three prequels, and *The Force Awakens*). They are ordered by film chronology, so Episode IV, *A New Hope*, is the fourth entry in the list. 

As we said, an ego graph is just a subgraph centered on a particular actor. So R2-D2's ego graph is just:

```{r}
r2 <- subgraph(g, c("R2-D2", names(neighbors(g, "R2-D2")))) # R2-D2's ego graph
```
...
Note that we use `layout_as_star()` for the layout, so that the ego is placed at the center of the star graph, surrounded by their alters. We can specify the star node using the `center` option.
...
First, we create a little function that creates the ego subgraph for node `v`:

```{r}
get.ego <- function(v, graph) {
  subgraph(graph, c(v, names(neighbors(graph, v))))
}
```

Second, we use `lapply` to extract the ego subgraph for each node in the network:

```{r}
ego.graphs <- lapply(V(g)$name, get.ego, graph = g)
names(ego.graphs) <- V(g)$name # naming list
head(ego.graphs)
```

The result is a `list` object with $|V| = `r length(ego.graphs)`$ ego subgraphs composed of each node's alters and their connections. 

Second, to find out the clustering coefficient of each node, we just `sapply` the `igraph` function `edge_density` to each member of the `ego.graphs` list:
...
Here we see that both Greedo and Jabba are pendant nodes (with degree 1), so it doesn't make sense to analyze their clustering coefficients, since their ego graphs are just isolated nodes!
...
Note that in this analysis, Luke has the lowest clustering coefficient ($C = 0.34$). This usually indicates an ego whose alters are partitioned into distinct clusters (and hence they are not connected to one another), and the ego is a mediator or *broker* between those clusters [@burt92].
...
#| fig-height: 8
#| fig-width: 8
#| fig-cap: "Luke's ego network with alter nodes colored by community assignment via Newman's leading eigenvector method."
set.seed(456)
N <- neighbors(g, "Luke")
...
Here we can see that Luke mediates between three densely connected clusters: The Rebel Pilot community on the left and the Obi-Wan, Leia, Chewbacca, Han Solo, and Droid communities on the right. Note that we didn't need to use the `layout_as_star` option because, when the ego's network has a community structure, the traditional spring embedding layout places the ego at the center.
...
Finally, from each ego's clustering coefficient (sometimes called the **local clustering coefficient** of each node), we can compute the graph's **global clustering coefficient**, which is just the average of this quantity across each node in the graph:
...
Indicating a fairly clustered graph. 

In `igraph`, we can use the function `transitivity` to compute the local and global clustering coefficients, which can be specified with the argument `type`. For the local version, the function also expects a list of vertices:
...
So to calculate $ES$ in our network, all we have to do is calculate the average degree of each ego graph and subtract that from the ego's degree. We can obtain the average degree of each ego using the first two `lapply` and `sapply` lines in the following:
...
Note that if two nodes have the same degree but different clustering coefficients, then the effective sizes of their networks will differ, with the node with the *larger* clustering coefficient having a *smaller* effective size.
...
Here we can see that even though both Obi-Wan and Red Leader have an ego network of the same size ($k = 7$), a larger proportion of Obi-Wan's contacts ($C = 0.76$) are connected to one another than Red Leader's ($C = 0.57$), which means that Obi-Wan's effective size ($ES = 2.4$) ends up being smaller than Red Leader's ($ES = 3.6$). 
...
As @everett_borgatti05 note, in an ego network, betweenness is determined by the number of paths of length two that involve the ego, that is, by the number of disconnected alters. 
...
```{r}
round(ego.bet("Luke", g), 2)
```

This says that Luke has a pretty high ego-network betweenness.

We can, of course, compute it for everyone in the network like before:

```{r}
round(sapply(V(g)$name, ego.bet, graph = g), 2)
```

This confirms our original impression of Luke as the highest-ego-network-betweenness character, with Leia in second place.
...
chew <- subgraph(g, c("Chewbacca", names(neighbors(g, "Chewbacca")))) # Chewbacca's ego graph
...
If we have information on the categorical attributes of ego's alters, we may be interested in how diverse ego's choices are across those attributes. 
...
Now, we will pick the vertex attribute `homeworld` and try to measure how diverse each character's ego network is on this score. 
...
This says that Padme has a fairly diverse ego network when it comes to Home World. 
...
```{r}
round(sapply(V(g)$name, blau, w = g, a = "homeworld", norm = TRUE), 3)
```
...
Diversity measures the extent to which ego connects to alters who are the same or different from *one another*. We may also want to get a sense of how *homophilous* an ego network is, namely, the extent to which ego connects to alters that are the same or different from *them*.
...
As we can see, Mace Windu is mostly surrounded by other men (like him), but Padme's network includes only one other woman, and the rest are people with a different gender presentation than hers (or have no discernible gender, like the droids).

As with the clustering coefficient, we can compute a graph-level index of homophily for a given attribute. This is given by the average EI index of nodes in the graph for that attribute.
...
This shows a slight preference for same-gender ties in the network. 


#### Normalized EI

Sometimes, we may want to take into account that the group sizes of different categories of people are unequal in the network. For instance, Star Wars is full of characters gendered as men, which means that any homophily index will penalize men as being more homophilous simply because there are more men around to form ties with.

@everett_borgatti12 [p.564-565] proposes an approach to normalizing the EI index to account for unequal group sizes, yielding the $NEI$. So instead of computing EI, they suggest calculating:
...
Interestingly, while most people's scores are attenuated towards zero in the normalized scale, R2-D2's becomes more extreme, going from weakly positive (demonstrating "gender" heterophily) to extreme negative (showing the same "gender" preference). 
...
This shows that our previous unnormalized average underestimates homophily in this network. Instead, there is a moderately strong tendency for characters to co-appear with others of the same gender classification once the imbalance favoring men is accounted for. 
...
1. Number of alters linked to ego of a different category from ego.
1. Number of alters not linked to ego of the same category as ego.
1. Number of alters not linked to ego of a different category from ego.

This yields a classic 2 by 2 table. 
...
It is clear that the EI only uses information from the first row (alters tied to ego) and is thus sensitive to group size imbalances because it ignores the other pieces of information ($c$ and $d$). 
...
This makes it clear that NEI uses information from all four cells; therefore, it is insensitive to group size imbalances.
...
This shows a positive tendency for same gender ties, net of the imbalance between men and other-gendered characters in the film. 
...
This shows that, after accounting for group sizes, most characters display slight to moderate preferences for same-gender ties, with the exception of Anakin, Padme, Taun We, Owen, Clegg, and Sun Rit. 
...
This suggests that about 70% of the characters tend to appear with the same gender as others (accounting for differences in the sizes of the different gender groups).
...
This also suggests a slight preference for same gender ties in this network.

#### Yule's Q

The point biserial correlation is a good measure of homophily, but comes at the expense of the simplicity of the EI index. If you favor a simpler approach that also makes use of all of the information in the 2 X 2 table, Yule's $Q$ is a good option [@borgatti_etal24, p. 159]:

$$
Q = \frac{ad-bc}{ad+bc}
$$

Like $r^{pb}$, $Q$ works like a correlation coefficient. It is at its maximum value of $Q = 1$ when ego has no connections to different-category others ($b=0$)---maximum homophily---and it is at a minimum of $Q = -1$ when ego has no connections to same-category others ($a=0$)---maximum heterophily. 
...
As we can see, Ki-Adi-Mundi and Captain Typho have an ego network that combines maximum homophily and homogeneity with respect to gender (both ego networks are composed of all men)--and therefore both receive the minimum score of $Q=-1$.
...
This also suggests a slight same-gender preference in the network after accounting for unbalanced group sizes. 
