...
- Second, we use the `all_shortest_paths` function to actually compute the number of shortest paths between $i$ and $j$. This function takes three inputs: (1) A graph object `x`, (2) a sender node id `i`, and (3) a receiver node id `j`, and returns a `list` of the paths between the sender and receiver nodes in the form of vectors of node ids defining each path as elements of a list called "vpaths."
