...
To calculate the average path length, first we sum all the rows (or columns) of the geodesic distance matrix. This vector (of the same length as the number of nodes) gives us the sum of the geodesic distance of each node to each of the nodes. Then we divide this vector by the number of nodes minus one (to exclude themselves) to create a vector of the *average* distance of each node to each of the other nodes. Then we take the average of this set of averages to get the graph's average shortest path length, which in this case equals L =  `r round(L, 1)`.  Meaning that on average, each person in the *Pulp Fiction* is separated by little less than three contacts (a fairly small world).
