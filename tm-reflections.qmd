---
title: "The Method of Reflections and Correspondence Analysis in Two-Mode Networks"
execute: 
  eval: true
  echo: true
  output: true
...
```{r}
    ...
    colnames(p.c) <- paste("Cr_", c(1:iter), sep = "")
...
This all look like slightly different versions of the same number! The reason for this is as @hidalgo_hausmann09 note, is that the scores begin to *converge* as the number of reflections increase. To get noticeable differences, we need to *scale* the higher reflection scores using the standard unit normalization:

$$
C^R =\frac{C^R-}{\sigma(C^R)}
$${#eq-norm}

Where $\bar{C^R}$ is the mean reflective score and $\sigma(C^R)$ is the standard deviation of the reflective scores. In `R` we can do this using the `scale` function:
...
We can see in @tbl-ref that in higher order reflections, the scores have converged to a steady value. 
