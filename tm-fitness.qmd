...
#| tbl-cap: "Status Scores for Persons"
   library(kableExtra)
   library(dplyr)
   s.tab <- data.frame(fc1 = fc1$p.s/max(fc1$p.s), 
                        fc2 = fc2$p.s/max(fc2$p.s),
                        fc3 = fc3$p.s/max(fc3$p.s),
                        fc4 = fc4$p.s/max(fc4$p.s)
                        ) |> 
      arrange(desc(fc1))
   s.tab
```
   kbl(s.tab, format = "pipe", digits = 3,
       align = c("l", "c", "c", "c", "c"),
       col.names = c("Person", "Bonacich", "Fitness (C)", "Fitness (R)", "Fitness (RC)")) %>% 
   kable_styling(bootstrap_options = c("hover", "condensed", "responsive")) %>%
   column_spec(1, bold = TRUE) |> 
   footnote(symbol = c("Scores normalized by dividing by the maximum.")) 