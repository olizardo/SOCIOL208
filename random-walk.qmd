...
The interpretation of the entries of the $\mathbf{M}$ matrix is also straightforward. For instance, if we start a message from node 2, we should expect it to take an average of $m_{2, 6} =$ `r round(M[2,6], 1)` steps to get to node 6 by following random walks in the network.
