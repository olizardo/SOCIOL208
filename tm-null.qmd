...
These are the biadjacency matrix entries stretched out into a long vector of length equals to the number of rows multiplied by the number of columns of the matrix. 
...
Below is a function called `make.swap` that implements this algorithm, called "curveball" [@neal_etal24]. The function takes the biadjacency matrix as input and repeats the process described above 500 times:


```{r}
   make.swap <- function(x, k = 500) {
      z <- 1 #initializing counter
      while(z <= k) {
         a <- sample(rownames(x), 1) #selecting random person 
         b <- sample(rownames(x), 1) #selecting random person
...
Note that once we fix the person and group degrees, all the degree correlations observed in the ensemble are negative (below zero). The observed value also does not look as extreme as with the Erdos-Renyi or fixed degree model for either rows or columns separately. Let's check the p-value:
...
While the observed value is not significant using the threshold of $p < 0.05$, it is just under the conventional threshold of significance at $p < 0.05$! Publication hopes survive. 
