...
First, we create a little function that creates the ego subgraph for node `x` from the `g` graph:

```{r}
   get.ego <- function(x){subgraph(g, neighbors(g, x))}
```

Second, we use `lapply` to extract the ego subgraph for node in the network:

```{r}
   ego.graphs <- lapply(V(g)$name, get.ego)
   head(ego.graphs)
```
