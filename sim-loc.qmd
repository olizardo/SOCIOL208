...
## Fully Normalized Similarity Metrics 
...
When the neighborhoods of the two nodes coincide (e.g., $q = 0$ and $r = 0$) then turns into $\frac{p}{\sqrt{p^2}} = \frac{p}{p} = 1.0$. 
...
For instance, consider our toy graph from our [previous discussion](sim-structequiv.qmd) showcasing four structurally equivalent sets of nodes:
...
p <- p + geom_node_point(aes(
  x = x, y = y,
  color = as.factor(cols)
), size = 12)
...
We can then extract our blocks just like we did with the Euclidean distance hierarchical clustering results in our discussion of [structural equivalence](sim-structequiv.qmd):
