...
A node has a high **random walk betweenness centrality**^[Also called **current flow betweenness** and **absorbing random walk betweenness**.] if it is likely to be a frequent intermediary on random walk diffusion processes that start and end with some other pair of nodes $i$ and $j$ in the graph. 
