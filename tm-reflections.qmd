...
$${#eq-grefq}
 ...
 refq says that the reflective score of a person *p*at iteration *q* is the sum of the reflective scores the groups they belong to at the $q-1$ iteration ($C^R_g(q-1)$) divided by their number of memberships $C^{R}_p(1)$. 
...
Now we can plot the persons and groups in their reflective score trajectory across the even reflections. The result looks like .
...
library(ggrepel)
library(cowplot)
p <- ggplot(data = HH.p.long, mapping = aes(
  x = Ref, y = k,
...
p <- p + geom_text_repel(
  data = filter(HH.p.long, Ref == "Ref_2"),
  aes(x = Ref, label = person),
  size = 4, nudge_x = -2
)
p <- p <- p + geom_text_repel(
  data = filter(HH.p.long, Ref == "Ref_20"),
  aes(x = Ref, label = person),
  size = 4, nudge_x = 2
)
...
p <- ggplot(data = HH.g.long, mapping = aes(
  x = Ref, y = k,
...
p <- p + geom_point(size = 3.5)
p <- p + geom_text_repel(
  data = filter(HH.g.long, Ref == "Ref_2"),
  aes(x = Ref, label = group),
  size = 4, nudge_x = -1
)
p <- p <- p + geom_text_repel(
  data = filter(HH.g.long, Ref == "Ref_20"),
  aes(x = Ref, label = group),
  size = 4, nudge_x = 1
)
p <- p + theme_minimal_grid(font_size = 14, line_size = 0)
