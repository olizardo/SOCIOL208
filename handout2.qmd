...
Line one creates an (empty) `list` object in `R`. The beauty of a list object is that it is an object that can hold other objects (vectors, matrices, `igraph` graph objects, etc.) as members (it can also have other lists as objects, with lists all the way down). 

The idea is to populate this initially list with the vectors of the in-neighbor ids of each node listed in the vector `yw`. Lines 2-8 do that using a simple `for` loop starring the `igraph` command `neighbors`, a function which takes three inputs: an `igraph` graph object, a node id, and the "mode" ("in" or "out" for directed graphs). Here we want the in-neighbors, so `mode` = "in".

Now we have a `list` object in `R` of length equal to the number of younger women (12 in this case) with each entry equal to the ids of those women's in-neighbors. Now we need to create a vector of the unique ids of these nodes:
...
The last step is just to "unlist" all of this node ids to create a simple vector. The `unlist` native `R` function does that for us, taking a list as input and returning all of the elements inside the list as output. Here we wrap that call in the `unique` native `R` function to eliminate repeats (common in-neighbors across women):
...
We use the native command `setdiff` to find the elements in vector yw_in that are not contained in the vector of young women ids yw.
...
We can now plot the incoming advice network to younger women (in red):

```{r}
#| echo: false
#| fig-cap: "Women lawyers advice network showing incoming links from outside the group"
#| fig-cap-location: margin
#| fig-height: 7
   library(ggraph)
   set.seed(123)
    p <- ggraph(g.r, layout = 'gem')
    p <- p + geom_edge_parallel(color = "darkgray", edge_width = 0.1,
                                arrow = arrow(
                                length = unit(2, 'mm')),
                                end_cap = circle(3, 'mm'), 
                                sep = unit(2, 'mm'))
