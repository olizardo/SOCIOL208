...
As you can see, the function we `apply` to each column of the adjacency matrix `A` just takes each column vector and subtracts the mean of the same vector (`x - mean(x)`).
...
Now that we have our centered adjacency matrix, all we need to do is play our HITS status game on it:
...
In the network adjacency matrix case, the vectors in questions are the *columns* of the adjacency matrix for each individual, recording each of the incoming ties to that person. So if $\mathbf{a}_{\bullet i}$ is the column vector recording the in-neighbors of $i$ and $\mathbf{a}_{\bullet j}$ is the column vector recording the in-neighbors of $j$, the covariance between nodes $i$ and $j$ is given by:

$$
\begin{equation}
cov(i,j) = \frac{\sum_{k=1}^N [(\mathbf{a}_{\bullet i} - \mathbf{\bar{a}}_{\bullet i})(\mathbf{a}_{\bullet j} - \mathbf{\bar{a}}_{\bullet j})]_k}{N-1}
\end{equation}
$$

Where $\mathbf{\bar{a}_{\bullet i}}$ indicates the mean of the entries in vector $\mathbf{a_{\bullet i}}$.
