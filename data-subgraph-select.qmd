...
Second, we need to collect the node ids of the people who point to these set of nodes in a `list` object; that is, each of these node's **in-neighbors**. For that, we use the `igraph` function `neighbors`, wrapped into the handy dandy `lapply` meta-function in `R`:

```{r}
yw_in <- lapply(yw, function(x) {
  neighbors(law_advice, v = x, mode = "in")
})
```

This creates a list of length equal to the number of nodes in the object `yw` containing vectors of the node ids of each of their in-neighbors as elements. The beauty of a list object is that it is an object that can hold other objects (vectors, matrices, `igraph` graph objects, etc.) as members (it can also have other lists as members, with lists all the way down).^[For a primer on how to work with `R` lists see [here](https://rstudio.github.io/r-manuals/r-intro/Lists-and-data-frames.html)]The argument "mode" ("in" or "out" for directed graphs), tells the `igraph` function `neighbors` which *kind* neighbors you want (not necessary for undirected graphs). Here we want the in-neighbors, so `mode` = "in".
...
p <- p + geom_node_point(aes(
  x = x, y = y,
  color = as.factor(net_status)
), size = 5)
p <- p + scale_color_manual(
  labels = c("Younger Women", "Other Lawyers"),
  values = c("red", "blue")
)
p <- p + theme_graph() + theme(
