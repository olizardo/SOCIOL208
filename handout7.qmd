...
This means that (unnormalized) closeness centrality works the way:
...
However, as @borgatti_everett97 note, if we want *normalized* closeness centralities, we can't use the off-the-shelf normalization for one-mode networks in `igraph` as it will give us non-sense results.

Instead, we need to normalize the closeness score for each node set by its theoretical maximum. For people, this is:

$$
M + 2N -2
$$

And for groups/evens this is:

$$
N + 2M - 2
$$

So in our case, we create a normalization vector with this quantities:

```{r}
   n.p <- ncol(A) + (2 * nrow(A)) - 2
   n.e <- nrow(A) + (2 * ncol(A)) - 2
   norm.vec <- c(rep(n.p, nrow(A)), rep(n.e, ncol(A)))
```
