...
We can adapt the code we used for the directed case for this purpose. First, we create an edge list data frame using the `igraph` function `as_data_frame`:
...
Note that by default, the `assortativity_degree` function in `R` only returns the out-in correlation for directed graphs:
