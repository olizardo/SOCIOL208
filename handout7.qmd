...
In `R` you can find the values that yield the eigendecomposition of any matrix (if one exists) using the function `eigen`. 
...
Now, the idea is that we can perform this eigendecomposition with any matrix, including a network adjacency matrix or a proximity matrix derived from it like the ones we used to calculate the PageRank Status scores earlier. In fact, we have already done that in part, because the status scores compute the first column (leading eigenvector) of the $\mathbf{U}$ matrix for any proximity or adjacency matrix you feed into it.
...
CA is based on this principle, and it works with the eigendecomposition of the matrices $\mathbf{P_{PP}}$ and $\mathbf{P_{GG}}$ as we defined them earlier:
