...
Note that the [one-mode projections](basic-tm-duality.qmd) can be considered unnormalized similarity matrices [just like in the case of regular networks](sim-loc.qmd). That means that if we have the degrees of nodes in each mode, we can transform this matrix into any of the **normalized vertex similarity** metrics we discussed before, including Jaccard, Cosine, Dice, LHN, and so on. 
