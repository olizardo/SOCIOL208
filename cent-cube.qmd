...
The three dimensional array can be thought of as a "cube" by stacking multiple reachability matrices between every pair $s$ and $r$ along a three dimensional dimension $b$. So each "b-slice" of the cube will contain the number of times node $b$ stands in a shortest path between $s$ and $r$ divided by the total number of paths between $s$ and $r$ which as you [recall](cent-bet.qmd) computes the pairwise dependence of $s$ and $r$ with respect to $b$.
...
```{r}
    ...
    S <- matrix(1, n, n)
    for (i in 1:n) {
        j <- setdiff(i:n, c(i, as.numeric(neighbors(w, i)))) # non-neighbors in upper triangle
        if (length(j) > 0) {
            S[i, j] <- 0 # Set sij to 0 for disconnected nodes
            vpaths <- all_shortest_paths(w, from = i, to = j)$vpaths
            if (length(vpaths) > 0) {
                dests <- unlist(lapply(vpaths, function(x) as.numeric(x[length(x)])))
                counts <- table(dests)
                S[i, as.numeric(names(counts))] <- as.numeric(counts)
                }
            }
        }
...
The function is called `nsp` and takes a graph `w` as input and returns and matrix called $\mathbf{S}$ with entries $s_{ij}$ equal to the total number of shortest paths between $i$ and $j$. This is done in three steps:

- First, we initialize an $n \times n$ matrix of ones called $S$ in line 3 to the all ones matrix. We will update the entries of this matrix with the actual number of shortest paths between each pair of nodes in the graph.

- Next, we loop through each node $i$ in the graph in line 4. For each node $i$, we identify the set of target nodes that are not neighbors of $i$ and are in the upper triangle of the matrix using the `neighbors` function in `igraph` and the `setdiff` function in base `R` in line 5. We then check if there are any target nodes to process. If there are, we set the default value of the number of paths to zero for those target nodes in line 6.

- Next, we compute the shortest paths from node $i$ to the target nodes using the `all_shortest_paths` function in `igraph` in line 7. This function returns a list of paths, where each path is represented as a vector of node ids. We check if there are any paths to process in line 7. If there are, we extract the destination nodes from the paths using `lapply` and `unlist` in line 8, and then count the number of paths to each destination node using the `table` function in line 9. Finally, we update the corresponding entries in the $S$ matrix with the counts of paths in line 10. After processing all nodes, we copy the upper triangle of the $S$ matrix to the lower triangle to ensure symmetry in line 12, and return the $S$ matrix.
