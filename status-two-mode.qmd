...
To see this, let's bring back our trusty status distribution game for one-mode networks:

```{r}
   status1 <- function(w, e = 1e-10) {
      n <- nrow(w) #number of actors
      x <- rep(1/n, n) #initial status vector set
      d <- 1 
      k <- 0 #initializing counter
      while (d > e) {
          o.x <- x #old status scores
          x <- w %*% x #new scores a function of old scores and adjacency matrix
          x <- x/norm(x, type = "E") #normalizing new status scores
...
Now we play the usual status game over the one mode projections. For people this would be:
...
Now to compute our double-normalized PageRank status scores, we just play the status game on the double-normalzied matrix and its tranpose:
