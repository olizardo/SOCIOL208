...
Recall that, according to @freeman79, the key conceptual distinction between [closeness](cent-clos.qmd) and betweenness is between (pun intended) the capacity to reach others quickly (e.g., via the shortest paths) and the capacity to *intermediate* among those same paths. High-betweenness nodes control the flow of information in the network between other pairs of nodes in the network when this information or resource flows via shortest paths.^[The lesson on [random walk centrality concepts](random-cent.qmd) covers variations of centrality that relax the assumption that information or resources flow strictly via shortest paths.]
