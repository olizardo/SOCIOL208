...
So to get the centered network adjacency matrix, we first need to compute the corresponding **column means** of the matrix. Note that this will be equivalent to each individual's **indegree** (the sum of the columns) divided by the total number of nodes in the network ($k^{in}/n$), which is kind of a normalized degree centrality score, except the usual normalizatio is to divide by $n-1$ as we saw in our discussion of [centrality](centrality.qmd).
...
Note that to subtract the column mean vector from each column of the adjacency matrix we first transpose it, do the subtraction, and "untranspose" back. We can use the `colMeans` function to check that the column means of the centered adjacency matrix are indeed zero:
...
Let's check that out in our case. The same package we used to compute the PCA of the adjacency matrix (`FactoMineR`)can be used to compute the correspondence analysis of any matrix in `R` using the function `CA`:
...
But what are the CA scores supposed to capture? Let's look at them side-by-side next to the SALSA hub and authority scores, as shown in @tbl-ha.
