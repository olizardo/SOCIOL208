...
@lancichinetti_etal09 describe an algorithm to determine what they call the "natural community" of a given node. The idea is to start with that node as its own subgraph, add the neighbor with the highest fitness relative to the starting subgraph, and continue adding subgraph neighbors with the highest fitness relative to the growing subgraph until the remaining neighbors have a fitness less than $\epsilon$ where $\epsilon$ is a small number (e.g., $\epsilon = 0.01$). The final subgraph thus constructed is the seed neighbor's natural community. 

First, let's write a function that takes a list of nodes specifying a subgraph of a larger graph as input and returns all unique neighbors of those nodes as output.
...
We can see from @fig-fitness-2 that indeed this set of nodes (pictured in blue) belongs to a cohesive subgroup proximate to node 31 (pictured in red). 
