...
In the lecture notes on [random walk concepts](basic-random.qmd), we defined two basic quantities helpful for analyzing networks in terms of dynamic diffusion processes: The **average first passage time** and the **average commute distance**. 

Both of these concepts serve as measures of distances (and their inverse proximity) based on how the two nodes are connected via direct and indirect paths, where the paths are defined by how something (e.g., gossip, disease, a meme) diffuses through the walk structure of the network based on transition probabilities governed by the inverse of the degrees of each node.

In the lecture on [closeness centrality](cent-clos.qmd), we saw how one of the fundamental centrality measures in social network analysis is defined according to a distance measure, in that case, the **geodesic** or **shortest path distance**. Two nodes are far if the length of the shortest path separating them is very long, and are close if it is very short (with the minimum being adjacency or length one).

Since random walk concepts are distance/proximity measures, we can use them to build analogs of centrality measures based on geodesic distance.

## Random Walk Closeness

The most obvious extension is to define a measure of closeness based on the random-walk concept of the average first-passage time. Two nodes are distant if the average number of steps required to get from one to the other is large, and they are close if the number of steps is small. So the average first-passage time metric is the random-walk analog of the geodesic distance, but it is based on random walks over *all* paths in the graph, regardless of length.
...
Where the quantity in brackets is just the sum of the rows of the matrix $\mathbf{M}$ for node $i$. 
...
First, we need the code we used [before](cent-random.qmd) to compute the average first passage time to get our random walk distance matrix $\mathbf{M}$:
...
And here are the first ten rows and columns of the $\mathbf{M}$ matrix:
...
Now to compute random walk closeness according to :
...
In contrast to measures based on shortest paths, in which the top characters also have the highest closeness, the random-walk version selects more obscure characters as the most central (e.g., The Gimp and Zed).

Note, however, that because the average first passage time is an asymmetric measure, we have a choice as to whether to compute it based on the "out" or the "in" distances. When we use the row sums of the $\mathbf{M}$ matrix, we count a node as close if the average first passage time between it and the other nodes in the graph is small. 
...
Using this measure yields more interpretable results, with the most important characters in the story appearing in the top ten.

## Current Flow Closeness

What if we used the average commute time as our distance measure instead? This makes sense, because just like the average first passage time, two nodes are far if the average commute time between them is big, and close if it is small. 

If we do that, we end up with a closeness centrality measure called **Current Flow Closeness**, which is a weird name because it was developed in the study of electrical circuit networks!
...
And here are the first ten rows and columns of the $\mathbf{N}$ matrix:
...
We can extend a similar "random walk" reasoning to the concept of [betweenness centrality](cent-bet.qmd). Recall that the standard computation of betweenness centrality only counts the number *shortest paths* that a given node is an intermediary on. We can extend the same reasoning beyond shortest paths to consider all paths of any length.

Consider a random walk process on a social network of the sort we [talked about before](basic-random.qmd). In this version, we select a "seed" or "source" node $i$ and a destination (also called "sink) node $j$. We start a diffusion process on $i$ and stop it when it reaches $j$ (this is called an **absorbing random walk**). The node-to-node transitions are governed by the probabilities stored in the matrix $\mathbf{P}$.

If we replay this process many times, each time selecting a random source and a random destination node in the network (such that all pairs of nodes get to play each role), then we *could* count the number of times the thing diffuses through the network when *through* some third node $k$. 

A node has a high **random walk betweenness centrality**^[Also called **current flow betweenness** and **absorbing random walk betweenness**.] If it is likely to be a frequent intermediary in random-walk diffusion processes that start and end with some other pair of nodes $i$ and $j$ in the graph. 

### Computing Random Walk Betweenness

How do we calculate random walk betweenness? Turns out the information we seek is stored in the **pseudo-inverse of the Laplacian** matrix ($\mathbf{L}^+$), which you recall from our [previous discussion](basic-random.qmd) is defined as:
...
$$
\phi_k(i,l) = \frac{1}{2}\sum_{j \in N(k)}^{N} a_{jk}|l_{ij}^+ - l_{jl}^+ - l_{ik}^+ + l_{kl}^+|
$$ {#eq-flow1}

Where the summation goes over all of $k$'s neighbors $j$. What this formula tells us is that the amount of flow that $k$ intermediates when a random walk process starts at $i$ and ends with $l$ is the sum of the amount of flow that would come through each edge $e_{jk}$ in the graph, connecting $k$ to one of its neighbors $j$.
...
The `flow.k` function takes the seed node `a`, the destination node `b`, the intermediary node `c`, and the graph `w` as arguments, and returns the total amount of flow that would go from each of $k$'s neighbors $j$ as output according to .
...
Now, the overall random walk betweenness centrality of any node is just its average flow score across each pair of nodes in the graph:
...
@fouss_etal16 [p. 162] show that we can use some matrix tricks to compute these quantities more efficiently (e.g., without looping). First, they note that the total flow going through a particular edge $e_{jk}$ in the graph for every source/destination pair of nodes can be computed as follows. 
...
This gives us the same answer as we obtained earlier for Vincent (the top node) while also showing the other top random walk betweenness centrality nodes in the network. These nodes, are not surprisingly, also some of the most central characters in the story. 
