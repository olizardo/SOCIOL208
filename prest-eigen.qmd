...
One way to think about this second version of well-placedness is that what is transmitted through the network is the *network itself*, or more accurately, the *importance*, *status*, and *prestige* of the people you are connected to, preferably flowing *from* them (high status people) to *you*. This perspective suggests that an actor's status is a direct reflection of the status of those who confer recognition or attention upon them.
...
Eigenvalues and eigenvectors, like Don Quixote and Sancho Panza, come in pairs, because you need a unique combination of both to solve the equation. Typically, a given matrix (like an adjacency matrix) will have multiple $\lambda/\mathbf{x}$ pairs that will solve the equation. Together, the whole set $\lambda/\mathbf{x}$ pairs that make @eq-eigen true are the **eigenvalues** and **eigenvectors** of the matrix. 
...
Imagine everyone has the same "quantum" of status to begin with (this can be stored in a vector of length equal to the number of actors in the network). Then, at each step, people "send" the same amount of status to all their alters in the network. Then we repeat, with everyone sending the amount of status they *now* have after receiving it from others. We repeat this many times. At the end of each step, we compute people's new status scores using @eq-status-sum. We stop doing this once people's status scores no longer change across iterations.

Let us see a real-life example at work. 

We will use a dataset collected by David Krackhardt on the friendships among 21 managers at a high-tech company on the West Coast (see the description [here](https://rdrr.io/github/schochastics/networkdata/man/ht_friends.html)). The data are reported as directed ties ($i$ nominates $j$ as a friend), but we will constrain ties to be undirected:
...
set.seed(123)
plot(g,
  vertex.size = 7, vertex.frame.color = "gray", edge.color = "gray",
  vertex.label.cex = 1.25, edge.arrow.size = 0.25,
...
status1 <- function(w) {
  ...
  while (d > 1e-10) {
    ...
    d <- sqrt(sum((x - o.x)^2)) # Euclidean distance between new and old status vectors
  } # end while loop
  return(as.vector(x))
}
```
...
kbl(eig.dat[1:10, ],
  format = "html", align = "c", row.names = FALSE,
  caption = "Top Ten Eigenvector Scores for a Directed Graph."
...
In a neat paper, @alvarez-socorro_etal15 combine ideas of [local similarity analysis](sim-loc.qmd) with prestige ranking to derive an interesting twist on the usual Eigenvector-based prestige score. 
...
How do we do it? First, we just need to compute one of the [many similarity metrics](sim-loc.qmd), to come up with a **dissimilarity matrix** between nodes. We choose the Jaccard similarity (because the Jaccard dissimilarity is a true metric distance), which can be computed using the following function:
...
Then @alvarez-socorro_etal15 creates a new matrix `W` which equals the **element-wise** product between the adjacency matrix and the distance matrix:
...
Of course, in some settings, it might make sense to say that people receive more status points by being connected to others who are themselves connected to others (similar others). This situation would apply when status is generated not by *brokerage*but by belonging to well-defined *cohesive groups*.
...
```{r}
prestige.sim <- function(w, delta = 0, gamma = 0) {
  A <- as.matrix(as_adjacency_matrix(w))
  A.p <- A %*% A
  A.q <- A %*% (1 - A)
  A.r <- (1 - A) %*% A
  S <- A.p / (A.p + A.q + A.r)
...
There is, of course, an even more intimate connection between vertex similarity analysis and prestige scoring at the node level in networks. The reason for this is that some forms of "global" vertex similarity analysis, like the **Katz similarity** [@katz53] discussed in the [global vertex similarities lecture notes](sim-glob.qmd), rely on the same mathematical apparatus and distributional network imagery as the prestige ranking scores like eigenvector centrality [@vigna16]. 
...
So here's another way to understand the "eigenvector centralities" computed by our status distribution game: Better-connected nodes by this metric are those who can reach all nodes in the graph via direct or indirect paths, with those who do so using shorter paths being better placed. 
...
Of course, `igraph` also has a dedicated function to compute the Katz centrality scores, called `alpha_centrality`. It takes a graph as input and returns the scores as output:

```{r}
    a <- 1 / eigen(A)$values[1] - 1e-05
    s.katz <- alpha_centrality(g, alpha = a)
    round(s.katz / max(s.katz), 3)
```

Note that you have to specify the alpha value to be close to the reciprocal of the first eigenvectors to get the same scores. 
...
This function takes the adjacency matrix (`w`) as input and returns the vector of Beta Centrality scores (`s`) as output, normalized to fall in the zero to one interval. We use min-max normalization here because a negative `beta` can result in negative scores, and this approach scales them appropriately. Note that if a value outside of the outlawed range is given for the parameter $\beta$, the function computes scores close to the eigenvector centrality ordering. 
...
Consider @fig-toy. According to the usual eigenvector centrality principle, we expect node 1 to have the highest eigenvector centrality score, as it is connected to other highly connected nodes. 
...
However, imagine instead that the colors in the graph stand for "sellers" (tan nodes) and "buyers" (blue nodes). Accordingly, node 1 is trying to get nodes 2, 3, and 4 (in light blue) to *buy* something from it (such as a house). However, each of the light blue nodes has *other* prospective sellers (four each). 
...
As expected, in the Beta Centrality scoring with a negative $\beta$ parameter, the blue nodes, who are connected to others who don't have many other options, are now the most central, and node 1, which is connected to the blue nodes with many options, is now the least central!
...
Which, as we can see, are the same scores we just calculated. 
