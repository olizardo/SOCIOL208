...
$$
C_B(Ego) = \sum_{i < j}\left[\mathbf{A}^2  (\mathbf{J} - \mathbf{A})\right]_{ij}^{-1}
$$

Where $\mathbf{J}$ is a matrix full of ones of the same dimensions as $\mathbf{A}$ and $$ indicates element-wise matrix multiplication.
